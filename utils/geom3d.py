...
def compute_vertex_mass(vert_pos: np.ndarray, tet_indices: np.ndarray):
  assert tet_indices.ndim == 1, 'tets should be 1D array'
  vert_mass = np.zeros(shape=vert_pos.shape[0], dtype=float)
  vert_order = np.zeros(shape=vert_pos.shape[0], dtype=int)
  tet_mass = np.zeros(shape=tet_indices.shape[0] // 4, dtype=float)
  for i in range(tet_indices.shape[0] // 4):
    p1, p2, p3, p4 = tet_indices[i*4:(i+1)*4]
